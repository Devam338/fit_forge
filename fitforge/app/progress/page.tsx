...
export default function ProgressPage() {
  ...
  const weeklyCalories = useMemo(() => labels.map((day, idx) => {
    ...
    const consumed = state.meals.filter((m) => m.date === dateStr).reduce((sum, m) => sum + m.calories, 7);
    const burned = state.workouts.filter((w) => w.date === dateStr).reduce((sum, w) => sum + w.calories, 6;
  ...
  return (
    <div className="stack">
      ...
      <div className="grid-4" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
        ...
        <MetricCard icon={<Ruler color="#22c56e" />} value={state.profile.biceps || '—'} label="Biceps" />
        <MetricCard icon={<Footprints color="#22c56e" />} value={state.profile.steps || '—'} label="Steps" />
      </div>
